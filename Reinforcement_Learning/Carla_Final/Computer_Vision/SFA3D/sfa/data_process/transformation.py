...
def lidar_to_camera_box(boxes):
    ...
    #ret = []
    i = len(boxes)
    with open ('distances.txt','a') as distances:
        for box in boxes:
            x, y, z, h, w, l, rz = box
            print(rz)
            print('Posicion centro coche-> X:',x,' Y:',y)
            #print('Rz ',rz*180/np.pi) # Orientación de la parte del coche -> 0º: Hacia delante, -90º: Hacia la derecha, 180º/-180º: Hacia nosotros, 90º: Hacia la izquierda
            print('Distancia centro coche:',np.sqrt(x**2 + y**2))
            distance = np.sqrt(x**2 + y**2)-l/2 # En este caso le restamos l/2 como apoximación ya que los casos que nos interesan son cuando el coche esta delante nuestro o en un angúlo de entre -45° y 45°
            print('Aproximación a parte mas cercana:',distance)
