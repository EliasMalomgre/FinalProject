...
from Carla_Final.Utils.utils import get_speed, get_trafficlight_trigger_location, \
    is_within_distance, compute_distance
from Carla_Final.agents.navigation.CustomPlanner import LocalPlanner, RoadOption
from Carla_Final.agents.navigation.global_route_planner import GlobalRoutePlanner
...
class CustomAgent(object):
    ...
    def run_step(self, action):
        ...
        control = self._local_planner.run_step(action)
        if hazard_detected:
    ...
    def distance_tool(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0,
                                   lane_offset=0):
        """
        Method to check if there is a vehicle in front of the agent blocking its path.
        ...
        def get_route_polygon():
            ...
            for wp in self._local_planner.get_waypoints():
                if ego_location.distance(wp.transform.location) > max_distance:
                    break
        ...
        for target_vehicle in vehicle_list:
            ...
            if (use_bbs or target_wpt.is_junction) and route_polygon:
                ...
                target_list = [[v.x, v.y, v.z] for v in target_vertices]
                target_polygon = Polygon(target_list)
                if (target_wpt.road_id == ego_wpt.road_id and target_wpt.lane_id != ego_wpt.lane_id):
                    continue
                if route_polygon.intersects(target_polygon) or route_polygon.contains(target_polygon):
                    possible_entries_list.append(target_vehicle)
                    #return (True, target_vehicle, compute_distance(target_rear_transform.location, ego_front_transform.location), ego_extent, route_polygon)

        if len(possible_entries_list)>0:
            closest_vehicle= None
            closest_distance = 200
            for vehicle in possible_entries_list:
                target_transform = vehicle.get_transform()
                target_forward_vector = target_transform.get_forward_vector()
                target_extent = vehicle.bounding_box.extent.x
                target_rear_transform = target_transform
                target_rear_transform.location -= carla.Location(
                    x=target_extent * target_forward_vector.x,
                    y=target_extent * target_forward_vector.y,
                )
                dist = compute_distance(target_rear_transform.location, ego_front_transform.location)
                if dist < closest_distance:
                    closest_distance = dist
                    closest_vehicle = vehicle
            return (True, closest_vehicle, closest_distance,ego_extent, route_polygon)

        return (False, None, -1, ego_extent, route_polygon)

    def detect_traffic_light(self, lights_list=None, max_distance=None):
        """
        Method to check if there is a red light affecting the vehicle.
        ...
        if self._last_traffic_light:
            if self._last_traffic_light.state != carla.TrafficLightState.Red:
                ...
                dist = trigger_wp.transform.location.distance(ego_vehicle_location)
                if dist < max_distance:
                    return (True, self._last_traffic_light, dist)
        ...
        for traffic_light in lights_list:
            ...
            if is_within_distance(trigger_wp.transform, self._vehicle.get_transform(), max_distance, [0, 90]):
                self._last_traffic_light = traffic_light
                dist = trigger_wp.transform.location.distance(ego_vehicle_location)
                return (True, traffic_light, dist)
