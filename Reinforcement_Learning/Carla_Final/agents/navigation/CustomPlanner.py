...
from Carla_Final.Utils.utils import draw_waypoints, get_speed
from Carla_Final.agents.navigation.CustomController import CustomVehicleController
...
class LocalPlanner(object):
    ...
    def __init__(self, vehicle, opt_dict={}, map_inst=None):
        ...
        self._dist_waypoints_queue = deque(maxlen=10000)
    ...
    def _init_controller(self):
        ...
        self._vehicle_controller = CustomVehicleController(self._vehicle,
    ...
    def run_step(self, action, debug=False):
        ...
        dist_num_waypoint_removed = 0
        self._dist_waypoints_queue = copy.copy(self._waypoints_queue)
        for waypoint, _ in self._waypoints_queue:
            ...
            if veh_location.distance(waypoint.transform.location) < 1:
                dist_num_waypoint_removed += 1
        ...
        if dist_num_waypoint_removed > 0:
            for _ in range(num_waypoint_removed):
                self._dist_waypoints_queue.popleft()
        ...
        if len(self._waypoints_queue) == 0:
            ...
            control = self._vehicle_controller.run_step(self._target_speed, self.target_waypoint, action)
    ...
    def get_waypoints(self):
        if len(self._dist_waypoints_queue) != 0:
            return list(zip(*self._dist_waypoints_queue))[0]
        else:
            return []
